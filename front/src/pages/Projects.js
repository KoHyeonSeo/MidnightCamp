...
import { INIT_MODEL_INFO, SET_SEARCH_KEY, SET_SEARCH_RESULT } from '../modules/ModelModule';
...
function Projects() {
    ...
    useEffect(
        async () => {
            ...
            // navigate(`/hephaistos/search?type=${ modelInfo[0].search_type }&keyword=${ modelInfo[0].search_key }`)
            console.log(result);
            await dispatch({ type: [SET_SEARCH_RESULT], payload: result});
        }, []
    );

    return (
        <div className={ MyPageStyle.mypage }>
            <div className={ MyPageStyle.title }>Projects</div>
            ...
            <div className={ MyPageStyle.list }>
                <ul>
                    { modelInfo[0].search_result.map((model, index) => 
                        {
                            <li><img className={ MyPageStyle.uncheck } src={require("../static/images/체크박스.png")}/>{model.nodeName}</li>
                        })
                    }
                </ul>
            </div>
            ...
            {/* <img className={ MyPageStyle.more } src={require("../static/images/더보기버튼.png")}/> */}
            {/* <img className={ MyPageStyle.uncheck } src={require("../static/images/체크박스.png")}/> */}
            {/* <img className={ MyPageStyle.check } src={require("../static/images/체크박스_채움.png")}/> */}
